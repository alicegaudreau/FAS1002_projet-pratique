...
#Conso energie
summary(full_db_tibble$primary_energy_consumption) #il y a un zero, c'est bizarre
...
summary(full_db_tibble$gas_production) #bon, vu que je regarde l"ensemble des donnees de differentes annees de production, la median est vraiment basse par rapport au maximum. Pour mes tableaux descriptifs, je vais me concentrer sur des annees en particulier. 
summary(full_db_tibble$coal_production)

stats_descriptives <- full_db_tibble %>% 
    select(population, Life.expectancy, gdp_per_capita, primary_energy_consumption, oil_prod_per_capita,gas_prod_per_capita,coal_prod_per_capita) %>% 
    pastecs::stat.desc()
...
donnees_Canada  <- subset(full_db_tibble, iso_code=='CAN') %>%
    select (year, hydro_share_elec, gas_share_elec, biofuel_share_elec, nuclear_share_elec, oil_share_elec, solar_share_elec, wind_share_elec, other_renewables_share_elec_exc_biofuel) %>% #petit tableau avec les variables dinteret
    pivot_longer(cols = hydro_share_elec:other_renewables_share_elec_exc_biofuel, names_to = "type_energy", values_drop_na = T) %>% #pour que le type denergie devienne une donnee et non une variable
    recode(value, 
          "hydro_share_elec" = "hydro", 
          "gas_share_elec" = "gaz", 
          "biofuel_share_elec" "biocarburants", 
          "nuclear_share_elec" = "nucléaire", 
          "oil_share_elec" = "pétrole", 
          "solar_share_elec" = "solaire", 
          "wind_share_elec" = "éolienne", 
          "other_renewables_share_elec_exc_biofuel" = "autres énergies renouvelables")


G2_Energy_Share_Canada <- ggplot(data = donnees_Canada) + geom_area(mapping = aes(x = year, y= value, na.rm=T, fill = type_energy)) +
  labs(title = "Graphique 2. Évolution du type \nd'énergie consommé au Canada",subtitle = "Données de 1985 à 2021", x = "Année", y = "pourcentage d'énergie") +
  theme(text = element_text(family = "Calibri"),
        plot.margin = unit(c(2,2,2,2), "cm"),
    plot.title = element_text(size = 20, hjust = 0.5, line = 0.8),
        plot.subtitle = element_text(size =12, hjust = 1, line = 0.8),
        axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12)) + 
    scale_colour_brewer(type = "seq", palette = "Spectral")

G2_Energy_Share_Canada
