...
fname <- paste(today(), "olympic_archive.zip", sep = "_")
...
Le fichier a ete telecharge ici : `r fpath`

```{r extraction}

unzip(zipfile = fpath, exdir = base_path)
```

Deux nouveaux fichiers CSV sont apparus apres le dezippage : `r fs::dir_ls(base_path)`
